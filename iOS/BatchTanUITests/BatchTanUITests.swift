...
final class BatchTanUITests: XCTestCase {
    ...
    func testAddEntryFromMainList() throws {
        ...
        let addButton = app.buttons["addHideButton"]
        XCTAssertTrue(addButton.waitForExistence(timeout: 5))
        addButton.tap()

        let firstField = app.textFields["speciesField"]
        XCTAssertTrue(firstField.waitForExistence(timeout: 5), "New entry sheet did not appear")
        firstField.tap()
        firstField.typeText("Test Entry")

        let saveButton = app.buttons["hideSaveButton"]
        ...
        for i in 0..<35 {
            ...
            if addButton.waitForExistence(timeout: 3) {
                ...
                if firstField.waitForExistence(timeout: 3) {
                    ...
                    app.buttons["hideSaveButton"].tap()
                }
            } else {
                break
            }
